...
while time_elapsed < 1000:
    for i in range(size):
        if i == 0:
            road_dt[i] = road_t[i] + (road_t[i] * 0.01/2 * (v(road_t[size - 1]) - v(road_t[i+1]))) + (v(road_t[i]) * 0.01/2 * (road_t[size - 1] - road_t[i+1]))
        elif i == size - 1:
            road_dt[i] = road_t[i] + (road_t[i] * 0.01/2 * (v(road_t[i-1]) - v(road_t[0]))) + (v(road_t[i]) * 0.01/2 * (road_t[i-1] - road_t[0]))
            if time_elapsed % 100 == 0:
                if i == 99:
                    print((road_t[i] * 0.01/2 * (v(road_t[i-1]) - v(road_t[0]))) + (v(road_t[i]) * 0.01/2 * (road_t[i-1] - road_t[0])))
        else:
